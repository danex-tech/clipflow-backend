...
// This identifies which Render server received the API request.
// SERVER_ID should be:
// Server A -> A
// Server B -> B
  ...
  try {
    ...
    const job = await downloadQueue.add(
      'download-job',
      {
        ...
        // Server that originally accepted the request.
...
router.get('/download/status/:jobId', async (req, res) => {
  try {
    ...
    // The worker writes serverId into progress when it starts.
    // This tells the frontend which server owns the final file.
    if (progress.serverId) {
      response.processingServer = progress.serverId;
    }
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // ----------------------------------------------------------
      // IMPORTANT SERVER OWNERSHIP CHECK
      // ----------------------------------------------------------

      /*
       * If Server A receives this request but Server B's worker
       * processed the job, Server A does NOT have the file.
       *
       * Tell the frontend which server it needs to contact.
       */

      if (serverId && serverId !== SERVER_ID) {
        return res.status(409).json({
          error: 'FILE_ON_OTHER_SERVER',
          serverId,
        });
      }
      ...
      res.download(
        ...
        (err) => {
          if (err) {
            ...
            // DO NOT DELETE.
            //
            // Browser downloads can be interrupted and resumed.
            // The file must remain available.
            return;
          }

          console.log(
            `[Download] File served for job ${job.id} | ` +
          ...
          // No fs.unlink().
          //
          // The processed file remains on disk.
        }
      );
    } catch (err) {
      console.error(
